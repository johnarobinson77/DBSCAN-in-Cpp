...
The code included here implements the popular DBSCAN clustering algorithm [1] in C++.  This implementation uses a k-d tree to accelerate the process of building the clusters.  The k-d tree code is also presented here.  Using a k-d tree improves the performance of the clustering in two essential ways.  
...
Second, it supports removing items from the k-d tree after the items are added to a cluster.  Each clustered item is considered only once and does not have to be tagged as having been previously added to a cluster.  Also, as the k-d tree decreases in size, the searches speed up.
...
1. Create an instance the DBSCAN class,
...
**KdTree.h**  contains the KdTree class.  This class is an API wrapper around the KdTreeExtras class and the KdNode class.  The KdTreeExtras class implements some special pick and search functions on the KdTree important to a high performance.
...
**DBSCAN.cpp** provides an example and test case for the DBSCAN implementation The test case is 1600 artificially clustered data with 400 points per clusters.  That data is presented to DBSCAN the results should therefore end up with a Clusters list that is 1600 in size and each cluster should have 400 values.
...
3. Loop on all items in the cluster
    ...
    3. When all the items in the current cluster have been searched against the dataset, exit the loop.  This means there are no more items in the dataset that are within a search window of any of the points in this cluster.
